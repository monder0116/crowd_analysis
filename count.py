...
def process_gender_detection(i, detections, frame, crossed_in, crossed_out,
                             efficient_net_model):
...
class AnnotationManager:
    ...
    def __init__(self, line_y_position, video_width, video_height):
        ...
        start, end = sv.Point(x=0,
                              y=line_y_position), sv.Point(x=video_width,
                                                           y=line_y_position)
        self.line_zone = sv.LineZone(start=start,
                                     end=end,
    ...
    def annotate_frame(self, frame, detections, crossed_in, crossed_out,
                       results):
        labels = []
        ...
        annotated_frame = self.box_annotator.annotate(scene=frame.copy(),
        ...
        cv2.putText(annotated_frame, f"Crossed In: {crossed_count_in}",
                    (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(annotated_frame, f"Crossed Out: {crossed_count_out}",
                    (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

        return annotated_frame


def main(model_path, yolov8_model_path, source_path, conf_thres, iou_thres,
         line_y_position):
    ...
    del video_info
    annotation_manager = AnnotationManager(line_y_position=line_y_position,
    ...
    for frame in frames_generator:
        ...
        detections = annotation_manager.tracker.update_with_detections(
            detections[filter_by_class])
        crossed_in, crossed_out = annotation_manager.line_zone.trigger(
            detections)
        results = {}

        with ThreadPoolExecutor(max_workers=6) as executor:
            future_to_index = {
                executor.submit(process_gender_detection, i, detections, frame, crossed_in, crossed_out, efficient_net_model):
                i
                for i in range(len(detections.tracker_id))
            }
        ...
        annotated_frame = annotation_manager.annotate_frame(
            frame, detections, crossed_in, crossed_out, results)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="YOLO and EfficientNetB0 Video Processing")
    parser.add_argument("--gender_model_path",
                        type=str,
                        required=True,
                        help="Path to the EfficientNet model file")
    ...
    parser.add_argument("--conf_thres",
                        type=float,
                        default=0.25,
                        help="Confidence threshold for YOLOv8")
    parser.add_argument("--iou_thres",
                        type=float,
                        default=0.45,
                        help="IoU threshold for YOLOv8")
    parser.add_argument("--line_y_position",
                        type=int,
                        required=True,
                        default=250,
                        help="Y-axis position for the line")
    args = parser.parse_args()

    main(args.gender_model_path, args.detector_model_path, args.source_path,
         args.conf_thres, args.iou_thres, args.line_y_position)
